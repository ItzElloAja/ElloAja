...
simple.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && mem.includes(simple.user.jid)) {
        simple.sendMessage(anu.jid, "Halo! Saya E-BOTZ akan membantu meringankan beban hidup anda seperti membuat Sticker, membuat logo *Dll* , Silahkan Ketik .menu Untuk Mengakses Bot!", "conversation")
      }
      if (!isWelkom) return
      if (anu.action == "add" && !mem.includes(simple.user.jid)) {
        ...
        sendButImage(mdata.id, wel, "©E-BOTZ", buff, but)
      }
      if (!isWelkom) return
      if (anu.action == "remove" && !mem.includes(simple.user.jid)) {
        ...
        sendButImage(mdata.id, out, "©E-BOTZ", buff, but)
      }
