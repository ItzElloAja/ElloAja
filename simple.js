...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		faketeks = '©〖E BOTZ™〗' //Ganti Aja Nama Luh
		const isUrl = (url) => {
...
//━━━━━━━━━━━━━━━[ BUTTON ]━━━━━━━━━━━━━━━━━//
        ...
        const listmsg = (from, title, desc, list) => { // ngeread nya pake rowsId, jadi command nya ga keliatan
            let po = simple.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "𝑴𝑬𝑵𝑼 E-BOTZ☕","footerText": `_HELLO KAK_ >.<
✋ 
_*SIMPLENYA*_ :
...
-𝐸-𝐵𝑂𝑇𝑍 𝐴𝑑𝑎𝑙𝑎𝒉 𝑈𝑝𝑑𝑎𝑡𝑒 𝐷𝑎𝑟𝑖 𝑃𝑒𝑚𝑎𝑦𝑦 𝐵𝑜𝑡𝑧
...
costum(buffer, sticker, Verived, `©E-BOTZ`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `©_E BOTZ_ *V : 2.889H*`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
simple.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... ©E-BOTZ' })
fs.unlinkSync(ran)
})
break
...
sendButLocation(from, teks, faketeks, but) 
break
...
sendButLocation(from, "_Silahkan Pilih Jangka Sewa Di Bawah_", faketeks, but, mek)
break
case 'sebulan':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButLocation(from, "SEWA 1 BULAN 10K", faketeks, but, mek)
break
case 'permanen':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButLocation(from, "SEWA PERMANEN 15K", faketeks, but, mek)
break
case 'gopay':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButLocation(from, "GOPAY : 089609927539", faketeks, but, mek)
break
case 'dana':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButLocation(from, "DANA : 089609927539", faketeks, but, mek)
break
case 'ovo':
simple.updatePresence(from, Presence.recording)
but = [
...
sendButLocation(from, "OVO : 089609927539", faketeks, but, mek)
break
...
simple.sendMessage(from, buffer, image, {quoted: ftrol, caption: 'Nih ©E-BOTZ'})
break
case 'hijaber':
...
reply(mess.wait)
buffer = await getBuffer(ini.result.url)
simple.sendMessage(from, buffer, video, {quoted: ftrol, caption: 'Nih ©E-BOTZ'})
break
case 'ukhti':
...
but = [
        ...
        sendButLocation(from, teks, '©Created : Yeni Bot', but, mek)
break
                ...
                case 'steel3d':
                case 'wallgravity':
                    if (args.length == 0) return reply(`Example: ${prefix + command} awok`)
                    txt1 = args[0]
                    txt2 = args[1]
...
sendButLocation(from, tod, faketeks, but, mek)
break
...
Sendbutdocument(from, tescok, "© E-BOTZ*", fs.readFileSync('./poto/menu.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./poto/image/thumbnaildokumen.jpg'), filename:`Z-BOT8.pdf`, pageCount: 9999999 }, 
[{ buttonId: `${prefix}menu`, buttonText: { displayText: '🐣OWNER🐣' }, type: 1 }] , {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nDownload Script`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/x-O0WHkv3uc`}}})
break
